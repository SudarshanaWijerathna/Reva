...
export default function GoogleButton({ text }: { text: string }) {
  // 1. FIX: Grab redirectPath from the context!
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        localStorage.setItem("user_name", userInfo.given_name || userInfo.name); 
        localStorage.setItem("user_picture", userInfo.picture); 

        // --- BULLETPROOF SUCCESS LOGIC ---
        const pathToGo = redirectPath;
        closeAuthModal();
        
        if (pathToGo) {
          window.location.href = pathToGo; // Hard redirect
        } else {
